...
function toast(t:string){ const b=byId<HTMLDivElement>("toast"); if(!b) return; const s=byId<HTMLSpanElement>("toastMsg"); if(s) s.textContent=t; b.style.display="block"; setTimeout(()=>b.style.display="none",1800); }
...
function showView(id:"view-review"|"view-settings"){
  const r=byId<HTMLDivElement>("view-review");
  const s=byId<HTMLDivElement>("view-settings");
  const b=byId<HTMLButtonElement>("btnBack");
  if(id==="view-review"){ r?.classList.remove("hidden"); s?.classList.add("hidden"); b?.classList.add("hidden"); }
  else { r?.classList.add("hidden"); s?.classList.remove("hidden"); b?.classList.remove("hidden"); }
}

// Modal confirm (Word taskpane sandbox lacks window.confirm reliably)
function confirmAsync(title:string,message:string){
  return new Promise<boolean>(res=>{
    const o=req<HTMLDivElement>("confirmOverlay");
    ...
    const no=req<HTMLButtonElement>("confirmCancel");
    const done=(v:boolean)=>{o.style.display="none"; ok.onclick=null; no.onclick=null; res(v);};
...
function rgbToHue({r,g,b}:{r:number;g:number;b:number}){ r/=255; g/=255; b/=255; const max=Math.max(r,g,b), min=Math.min(r,g,b), d=max-min; let h=0; if(d===0) h=0; else if(max===r) h=((g-b)/d)%6; else if(max===g) h=(b-r)/d+2; else h=(r-g)/d+4; h=Math.round(h*60); if(h<0) h+=360; return h; }
function hueToEmoji(h:number){ const pals=[{h:0,e:"🟥"},{h:30,e:"🟧"},{h:60,e:"🟨"},{h:120,e:"🟩"},{h:210,e:"🟦"},{h:280,e:"🟪"}]; let best=pals[0], diff=999; for(const p of pals){ const d=Math.min(Math.abs(h-p.h),360-Math.abs(h-p.h)); if(d<diff){diff=d;best=p;} } return best.e; }
...
const DEFAULT_SETS: PersonaSet[] = [
  { id:"cross-functional-team", name:"Cross-Functional Team", personas:[
    P("Senior Manager","Senior manager prioritizing clarity, risk, outcomes.","Assess clarity of goals, risks, outcomes; give concise suggestions.","#fde047"),
    P("Legal","Corporate counsel focused on compliance.","Flag risky or ambiguous claims; suggest safer wording.","#f9a8d4"),
    P("HR","HR partner focused on inclusivity.","Spot exclusionary tone; suggest inclusive language.","#5eead4"),
    P("Technical Lead","Engineering lead, pragmatic.","Check feasibility, gaps, technical risks.","#93c5fd"),
    P("Junior Analyst","Detail-oriented analyst.","Call out unclear logic and missing data.","#86efac"),
  ]},
  { id:"marketing-focus-group", name:"Marketing Focus Group", personas:[
    ...
    P("Tech Pro","Precise & detail-oriented.","React to vague claims; ask for specifics.","#ef4444"),
  ]},
  // (You can add the other previously-discussed sets back here; keeping two for brevity & stability)
];
...
function loadSettings():AppSettings{
  try{
    ...
    const v=JSON.parse(s) as AppSettings;
...
function populatePersonaSets(){
  ...
  if(sel){
    sel.innerHTML="";
    sets.forEach(s=>{ const o=document.createElement("option"); o.value=s.id; o.textContent=s.name; sel.appendChild(o); });
  ...
  if(ssel){
    ssel.innerHTML="";
    sets.forEach(s=>{ const o=document.createElement("option"); o.value=s.id; o.textContent=s.name; ssel.appendChild(o); });
...
function renderPersonaNamesAndLegend(){
  ...
  const names=byId<HTMLSpanElement>("personaList");
  if(names) names.textContent=set.personas.filter(p=>p.enabled).map(p=>p.name).join(", ");
...
function renderPersonaEditor(){
  const set=currentSet();
  const box=byId<HTMLDivElement>("personaEditor");
  if(!box) return;
  box.innerHTML="";
  set.personas.forEach((p,idx)=>{
    ...
    card.innerHTML=`
      ...
      <div style="margin-top:6px;display:flex;gap:8px;align-items:center;">
        <label style="min-width:90px;">System</label>
        <input style="flex:1" type="text" id="pe-sys-${idx}" value="${escapeHtml(p.system)}"/>
      </div>
      <div style="margin-top:6px;display:flex;gap:8px;align-items:center;">
        <label style="min-width:90px;">Instruction</label>
        <input style="flex:1" type="text" id="pe-ins-${idx}" value="${escapeHtml(p.instruction)}"/>
      </div>
    `;
...
Office.onReady(async ()=>{
  ...
  const sel=byId<HTMLSelectElement>("personaSet");
  if(sel) sel.onchange=(e)=>{ SETTINGS.personaSetId=(e.target as HTMLSelectElement).value; saveSettings(); populatePersonaSets(); };

  const prv=byId<HTMLSelectElement>("provider");
  if(prv) prv.onchange=(e)=>{ SETTINGS.provider.provider=(e.target as HTMLSelectElement).value as Provider; hydrateProviderUI(); saveSettings(); };
  const key=byId<HTMLInputElement>("openrouterKey");
  if(key) key.oninput=(e)=>{ SETTINGS.provider.openrouterKey=(e.target as HTMLInputElement).value; saveSettings(); };
  const mdl=byId<HTMLInputElement>("model");
  if(mdl) mdl.oninput=(e)=>{ SETTINGS.provider.model=(e.target as HTMLInputElement).value; saveSettings(); };

  const ssel=byId<HTMLSelectElement>("settingsPersonaSet");
  if(ssel) ssel.onchange=(e)=>{ SETTINGS.personaSetId=(e.target as HTMLSelectElement).value; saveSettings(); populatePersonaSets(); };

  const saveSettingsBtn=byId<HTMLButtonElement>("saveSettings");
  if(saveSettingsBtn) saveSettingsBtn.onclick=()=>{ const set=currentSet(); set.personas.forEach((p,idx)=>{ const en=byId<HTMLInputElement>(`pe-enabled-${idx}`); const sys=byId<HTMLInputElement>(`pe-sys-${idx}`); const ins=byId<HTMLInputElement>(`pe-ins-${idx}`); const col=byId<HTMLInputElement>(`pe-color-${idx}`); if(en) p.enabled=en.checked; if(sys) p.system=sys.value; if(ins) p.instruction=ins.value; if(col) p.color=col.value||p.color; }); saveSettings(); renderPersonaNamesAndLegend(); toast("Settings saved"); };

  const restoreBtn=byId<HTMLButtonElement>("restoreDefaults");
  if(restoreBtn) restoreBtn.onclick=()=>{ const id=currentSet().id; const fresh=DEFAULT_SETS.find(s=>s.id===id); if(fresh){ const i=SETTINGS.personaSets.findIndex(s=>s.id===id); SETTINGS.personaSets[i]=JSON.parse(JSON.stringify(fresh)); saveSettings(); populatePersonaSets(); toast("Default persona set restored"); } };
  ...
  const exportBtn=byId<HTMLButtonElement>("exportBtn"); if(exportBtn) exportBtn.onclick=handleExportPDF;
...
function setBadgesHost(personas:Persona[]){
  const host=byId<HTMLDivElement>("personaStatus"); if(!host) return;
  host.innerHTML="";
  personas.forEach(p=>{
    const row=document.createElement("div");
    row.style.display="flex";
    ...
    row.innerHTML=`<span style="display:inline-flex;align-items:center;gap:6px;"><span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:${p.color};"></span>${escapeHtml(p.name)}</span><span id="badge-${p.id}" class="badge">queued</span>`;
    host.appendChild(row);
  });
}
function setBadge(id:string, st:PersonaRunStatus, note?:string){
  const b=byId<HTMLSpanElement>(`badge-${id}`); if(!b) return;
  b.className="badge "+(st==="done"?"badge-done":st==="error"?"badge-failed":"");
  b.textContent=st+(note?` – ${note}`:"");
}
...
async function handleRetryFailed(){
  if(RUN_LOCK){ toast("Already running…"); return; }
  RUN_LOCK=true;
  try{ await runAllEnabledPersonas(true); }
  finally{ RUN_LOCK=false; }
}
...
function seedFrom(s:string,which:"first"|"middle"|"last",words:number){
  ...
  const mid=Math.floor(t.length/2), half=Math.floor(words/2);
...
async function findRangeForQuote(ctx:any, quote:string):Promise<any|null>{
  ...
  const trySearch = async (needle:string) => {
    const r=body.search(needle,{matchCase:false,matchWholeWord:false,matchWildcards:false,ignoreSpace:true,ignorePunct:true});
    r.load("items"); await ctx.sync(); return r.items.length? r.items[0] : null;
  };
...
function withTimeout<T>(p:Promise<T>, ms=45000){
  return new Promise<T>((res,rej)=>{ const t=setTimeout(()=>rej(new Error(`Request timed out after ${ms}ms`)),ms); p.then(v=>{clearTimeout(t);res(v);},e=>{clearTimeout(t);rej(e);}); });
}
async function fetchJson(url:string,init:RequestInit){
  const r=await withTimeout(fetch(url,init));
  let body:any=null; let text="";
  try{
    const ct=r.headers.get("content-type")||"";
    if(ct.includes("application/json")) body=await r.json();
    else { text=await r.text(); try{ body=JSON.parse(text);}catch{} }
  } catch(e){ try{ text=await r.text(); }catch{} }
...
function parseJsonFromText(text:string){
  ...
  catch{ log("[PF] JSON parse error; full text follows",{text}); throw new Error("Model returned non-JSON. See Debug for raw output."); }
}
...
function scoreBar(label:string,val:number){
  const pct=Math.max(0,Math.min(100,val|0)); const c=barColor(pct);
  return `
...
// ===== Results bookkeeping (single implementation) =====
